...
const Section1: React.FC<{ scrollYProgress: MotionValue<number>; children: React.ReactNode }> = ({ scrollYProgress, children }) => {
  ...
  return (
    <motion.section
      style={{ scale, rotate }}
      className="sticky top-0 h-screen bg-[#030303] flex flex-col items-center justify-center text-white overflow-hidden origin-top"
    >
      {children}
    </motion.section>
  );
...
const Section2: React.FC<{ scrollYProgress: MotionValue<number>; children: React.ReactNode }> = ({ scrollYProgress, children }) => {
  ...
  return (
    <motion.section
      style={{ scale, rotate }}
      className="relative h-screen bg-[#030303] text-white flex flex-col items-center justify-center overflow-hidden origin-bottom z-10 shadow-2xl"
    >
